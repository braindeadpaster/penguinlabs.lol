...
import { currentUser, loadProfile, saveProfile, logout } from "@/lib/store";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setProfile(loadProfile(user) ?? emptyProfile(user));
  }, [router]);

  if (!profile) return <div className="min-h-[100dvh] bg-ink-950" />;
    ...
    <div className="min-h-[100dvh]">
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-white/10 bg-ink-950/80 px-5 py-3 backdrop-blur">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={save}
            className="rounded-lg px-4 py-2 text-xs font-bold text-black transition hover:opacity-90"
                ...
                <div key={i} className="flex items-center gap-2">
                  ...
                  <RemoveButton
                    onClick={() =>
                      set(
                        "socials",
                        profile.socials.filter((_, idx) => idx !== i)
                      )
                    }
                  />
                </div>
          ...
          <Section title="Music (optional)">
            <Field label="Track title">
              <TextInput
                ...
                onChange={(v) =>
                  set("music", { ...normMusic(profile), title: v })
                }
              />
            </Field>
            <Field label="Artist">
              <TextInput
                ...
                onChange={(v) =>
                  set("music", { ...normMusic(profile), artist: v })
                }
              />
            </Field>
            <Field label="Audio URL (mp3)">
              <TextInput
                ...
                onChange={(v) =>
                  set("music", { ...normMusic(profile), url: v })
                }
...
// Re-mount the preview when meaningful visual inputs change so effects (typewriter,
// particles) reset cleanly rather than accumulating.
