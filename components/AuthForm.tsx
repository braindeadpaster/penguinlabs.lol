...
export default function AuthForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError("");
    setBusy(true);
    const res = mode === "register" ? await register(username, passcode) : await login(username, passcode);
    setBusy(false);
    if (res.ok) {
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-7 backdrop-blur-sm">
          ...
          <form onSubmit={submit} className="flex flex-col gap-3">
            <label className="flex flex-col gap-1.5">
              <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-white/40">Username</span>
            ...
            <label className="flex flex-col gap-1.5">
              <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-white/40">Passcode</span>
              <input
                type="password"
                value={passcode}
            ...
            <button
              type="submit"
              disabled={busy}
              className="mt-2 rounded-xl py-3 text-sm font-bold text-black transition hover:opacity-90 active:scale-[0.99] disabled:opacity-50"
              style={{ background: ACCENT }}
            >
              {busy ? "…" : mode === "register" ? "Create my page" : "Log in"}
            </button>
          </form>
        </div>

        <p className="mt-5 text-center text-[11px] leading-relaxed text-white/30">
          Your passcode is hashed before it&apos;s stored. Still, use a unique one.
        </p>
      </div>
    </main>
  );
